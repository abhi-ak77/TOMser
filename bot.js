...
const {Message, StringSession, Image, Video} = require('./TOM/');
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.AFPLK == 'TOMser ' || config.AFPLK == 'ajfx' || config.AFPLK == 'vava' || config.AFPLK == 'TOMser ') {
        //thanks to afnanplk
        console.log(
            chalk.green.bold('thanks for watching -key cofirmed-')
        );
         }
         else if (config.AFPLK !== 'TOMser ' || config.AFPLK !== 'ajfx' || config.AFPLK !== 'vava' || config.AFPLK !== 'TOMser ') {
         console.log(
        ...
        console.log(
            chalk.green.bold('TOMSER WORK ' + config.WORKTYPE + ' NOW✨🌈'));
          // thanks to afnanplk
	    if (config.LANG == 'EN' || config.LANG == 'ML') {
                ...
                if (commits.total === 0) {
                    ...
                    await conn.sendMessage(
                        conn.user.jid,
                        '*🌟TOMSER UPDATE🌟*' + degisiklikler + '```', MessageType.text
                    ); 
                } 
          }
    });
    setInterval(async () => { 
        ...
        while (getGMTh == 9 && getGMTm == 01) {
            ...
            return await conn.sendMessage(conn.user.jid, '*[ DAILY ANNOUNCEMENTS TOMSER ]*\n\n' + announce, MessageType.text);
        }
...
*/
                        ...
                        try {
                            ...
                            if (config.NOLOG === 'off') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ TOMser  ~______~*' +
